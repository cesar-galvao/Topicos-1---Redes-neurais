...
A interpretação deste modelo fica muito mais complexa a partir da expressão algébrica anterior. Mantendo $x_2$ constante em sua média, i.e. zero, o comportamento de $\hat{y}$ em função de $x_1$ está representado na @fig-efeitox1 a seguir[^3]:
...
efeitos_x1 <- tibble(
  ...
  x2 = rep(0, 100)) %>% 
  mutate(yhat = ffwd(., back_validacao$best_theta)$yhat)
```
...
ggplot(efeitos_x1, aes(x = x1, y = yhat))+
  geom_line()+
  scale_x_continuous(breaks = c(-3, -2, -1, 0, 1, 2, 3))+
  theme_bw()+
  labs(x = TeX("$x_1$"), y = TeX("$\\hat{y}$"))+
  theme(panel.border = element_blank(), # Remove all panel borders
    axis.line = element_line(color = "#474747"), # Add axis lines
    axis.ticks.x = element_line(color = "#474747"), # X axis ticks
    axis.ticks.y = element_line(color = "#474747"), # Y axis ticks
